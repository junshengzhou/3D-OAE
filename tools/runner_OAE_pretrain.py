...
def run_net(args, config, train_writer=None, val_writer=None):
    ...
    for epoch in range(start_epoch, config.max_epoch + 1):
        ...
        for idx, (taxonomy_ids, model_ids, data) in enumerate(train_dataloader):
            ...
            out_points, masked_shape, vis_shape = base_model(points, cutmix = False)
            
            if config.model.use_cutmix:
                out_points_cutmix, masked_shape_cutmix, vis_shape_cutmix = base_model(points, cutmix = True)

            cd_loss = ChamferDisL1(masked_shape, out_points) 
            if config.model.use_emd:
                cd_loss = emd(masked_shape, out_points) 
            else:
                cd_loss = ChamferDisL1(masked_shape, out_points) 

            if config.model.use_cutmix:
                cd_loss = cd_loss + ChamferDisL1(masked_shape_cutmix, out_points_cutmix)
            ...
            if args.distributed:
                cd_loss = dist_utils.reduce_tensor(cd_loss, args)
                losses.update([cd_loss.item()])
            else:
                losses.update([cd_loss.item()])
